...
function renderStats() {
  ...
  if (foundationStatsContainer) {
    ...
    if (fTotal > 0) {
      ...
      if (fCompleted < fTotal) {
        ...
        var covRow = document.createElement('div');
        covRow.className = 'stats-bar-row';
        covRow.innerHTML = '<span style="font-size:11px;color:var(--green);padding:4px 0">Foundation Course Complete! ~' + (typeof getFoundationCoverage === 'function' ? getFoundationCoverage().foundationCoveragePercent : '29') + '% Quran word-token coverage</span>';
        foundationStatsContainer.appendChild(covRow);
      }
    } else {
  ...
  if (healthContainer) {
    ...
    var healthItems = [
      ...
      { label: 'Review Ease', value: String(srsStats.avgEaseFactor.toFixed(2)), pct: Math.round((srsStats.avgEaseFactor / 3) * 100), color: 'var(--blue)' },
