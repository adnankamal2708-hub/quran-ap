...
// Completing all 10 lessons recognizes ~29% of all Quran word tokens (real token-level coverage).
...
function buildFoundationCourse() {
  ...
  for (var li = 0; li < FOUNDATION_LESSON_COUNT; li++) {
    ...
    var lessonContexts = [
      ...
      'Final review of all 100 foundation words. After this you recognize approximately {coverage}% of all Quranic word occurrences.',
    ];
...
// ═══════════════════════════════════════════════════════════════
// REAL token-level coverage (replaces the legacy w.occ formula)
// Coverage percentages shown to users are computed from actual
// token-level matching against the real Quran corpus, using the same
// normalization as the occurrence index (OCCURRENCE_INDEX_NORM) and
// the per-key token counts generated by scripts/build-occurrence-index.js.
// The legacy w.occ formula inflated percentages (e.g. the old "84 percent
// of the Quran" claim); the real measured coverage is ~34% for the full
// 930-word set and ~29% for the Foundation Course's 100 words.
// ═══════════════════════════════════════════════════════════════
var _realCorpusStats = null;

function _getCoverageNormFn() {
  if (typeof window !== 'undefined' && typeof window.OCCURRENCE_INDEX_NORM === 'function') return window.OCCURRENCE_INDEX_NORM;
  if (typeof OCCURRENCE_INDEX_NORM === 'function') return OCCURRENCE_INDEX_NORM;
  return null;
}

function _getCoverageCorpus() {
  if (typeof QURAN_TEXT !== 'undefined' && QURAN_TEXT) return QURAN_TEXT;
  if (typeof __QURAN_TEXT !== 'undefined' && __QURAN_TEXT) return __QURAN_TEXT;
  return null;
}

/**
 * Real corpus stats: { totalTokens, counts } where counts maps each
 * normalized form (matching vocabulary words) to its token count.
 * counts is null when no real data is available (callers then fall
 * back to the legacy w.occ formula, e.g. in corpus-less test envs).
 */
function getRealCorpusStats() {
  if (_realCorpusStats) return _realCorpusStats;
  _realCorpusStats = { totalTokens: 0, counts: null };
  var counts = null;
  var total = 0;
  if (typeof OCCURRENCE_INDEX_TOKEN_COUNTS !== 'undefined' && OCCURRENCE_INDEX_TOKEN_COUNTS) {
    counts = OCCURRENCE_INDEX_TOKEN_COUNTS;
  } else if (typeof window !== 'undefined' && window.OCCURRENCE_INDEX_TOKEN_COUNTS) {
    counts = window.OCCURRENCE_INDEX_TOKEN_COUNTS;
  }
  if (typeof OCCURRENCE_INDEX_TOTAL_TOKENS !== 'undefined' && OCCURRENCE_INDEX_TOTAL_TOKENS > 0) {
    total = OCCURRENCE_INDEX_TOTAL_TOKENS;
  } else if (typeof window !== 'undefined' && window.OCCURRENCE_INDEX_TOTAL_TOKENS > 0) {
    total = window.OCCURRENCE_INDEX_TOTAL_TOKENS;
  }
  if (!counts) {
    // Dev/test fallback: scan the full corpus if it is synchronously available.
    var norm = _getCoverageNormFn();
    var corpus = _getCoverageCorpus();
    if (norm && corpus) {
      var scanned = {};
      for (var s in corpus) {
        var verses = corpus[s] && corpus[s].verses ? corpus[s].verses : [];
        for (var vi = 0; vi < verses.length; vi++) {
          var v = verses[vi];
          if (!v || !v.text) continue;
          var toks = String(v.text).split(/\s+/);
          for (var ti = 0; ti < toks.length; ti++) {
            if (!toks[ti]) continue;
            total++;
            var key = norm(toks[ti]);
            if (key) scanned[key] = (scanned[key] || 0) + 1;
          }
        }
      }
      counts = scanned;
  ...
  _realCorpusStats = { totalTokens: total, counts: counts };
  return _realCorpusStats;
}

/**
 * Real token count for a single word, or null when the corpus data is unavailable.
 */
function getRealWordTokenCount(w) {
  var stats = getRealCorpusStats();
  if (!stats.counts) return null;
  if (!w || !w.arabic) return 0;
  var norm = _getCoverageNormFn();
  var key = norm ? norm(w.arabic) : '';
  return key ? (stats.counts[key] || 0) : 0;
}

/**
 * Real coverage percentage for a SET of words (union-consistent: each token
 * is counted once via its normalized form). Returns null when unavailable.
 */
function getRealWordsCoveragePercent(wordList) {
  var stats = getRealCorpusStats();
  if (!stats.counts) return null;
  ...
  for (var i = 0; i < wordList.length; i++) {
    var w = wordList[i];
    if (!w || !w.arabic) continue;
    var key = norm(w.arabic);
    if (key && !seen[key]) { seen[key] = true; covered += stats.counts[key] || 0; }
  }
  return stats.totalTokens > 0 ? (covered / stats.totalTokens * 100) : 0;
}
...
function getRealFoundationCoveragePercent() {
  var fWords = typeof getAllFoundationWords === 'function' ? getAllFoundationWords() : [];
  return getRealWordsCoveragePercent(fWords);
}

/**
 * Average REAL token count per foundation word (for milestone estimates).
 * Returns 0 when the corpus data is unavailable.
 */
function getRealAverageFoundationWordTokenCount() {
  var fWords = typeof getAllFoundationWords === 'function' ? getAllFoundationWords() : [];
  if (!fWords.length) return 0;
  var total = 0;
  for (var fi = 0; fi < fWords.length; fi++) {
    var c = typeof getRealWordTokenCount === 'function' ? getRealWordTokenCount(fWords[fi]) : null;
    if (c === null || c === undefined) return 0;
    total += c;
  }
  return total / fWords.length;
}
...
function calculateCoverage() {
  ...
  // REAL coverage: token-level matching against the actual Quran corpus
  // (union of normalized forms of mastered words / total corpus tokens).
  var realStats = typeof getRealCorpusStats === 'function' ? getRealCorpusStats() : null;
  if (realStats && realStats.counts && realStats.totalTokens > 0) {
    totalOcc = realStats.totalTokens;
    var norm = _getCoverageNormFn();
    var seenKeys = {};
    for (var ci = 0; ci < allCanonical.length; ci++) {
      var w = allCanonical[ci];
      if (!mastered[w.id]) continue;
      masteredCount++;
      if (w.arabic && norm) {
        var key = norm(w.arabic);
        if (key && !seenKeys[key]) {
          seenKeys[key] = true;
          masteredOcc += realStats.counts[key] || 0;
        }
      }
    }
  } else {
    // Legacy fallback (no corpus data / test environments)
    totalOcc = getTotalQuranOccurrences();
    for (var lci = 0; lci < allCanonical.length; lci++) {
      var lw = allCanonical[lci];
      if (mastered[lw.id]) {
        masteredCount++;
        masteredOcc += lw.occ || 0;
      }
    }
  }
  ...
  // Coverage is now REAL token coverage (max ~24% for the full set), so
  // comprehension is anchored to the real scale — recalibration may be
  // warranted separately; this curve shape is unchanged.
...
function getFoundationCoverage() {
  ...
  // REAL token-level coverage for the foundation word set (see calculateCoverage).
  var realStats = typeof getRealCorpusStats === 'function' ? getRealCorpusStats() : null;
  if (realStats && realStats.counts && realStats.totalTokens > 0) {
    totalOcc = realStats.totalTokens;
    var norm = _getCoverageNormFn();
    ...
    for (var fi = 0; fi < fWords.length; fi++) {
      var w = fWords[fi];
      fTotalOcc += getRealWordTokenCount(w) || 0;
      if (!mastered[w.id]) continue;
      fMastered++;
      if (w.arabic && norm) {
        var key = norm(w.arabic);
        if (key && !seenKeys[key]) {
          seenKeys[key] = true;
          fMasteredOcc += realStats.counts[key] || 0;
        }
      }
    }
  } else {
    // Legacy fallback (no corpus data / test environments)
    totalOcc = getTotalQuranOccurrences();
    for (var lfi = 0; lfi < fWords.length; lfi++) {
      var lw = fWords[lfi];
      fTotalOcc += lw.occ || 0;
      if (mastered[lw.id]) {
        fMastered++;
        fMasteredOcc += lw.occ || 0;
      }
    }
  }
...
function getMilestoneStatus(coveragePercent) {
  ...
  if (nextMilestone) {
    // Estimate words needed: each word adds roughly its real token count to coverage
    var neededCoverage = nextMilestone.pct - coveragePercent;
    var realStats = typeof getRealCorpusStats === 'function' ? getRealCorpusStats() : null;
    var totalOcc = (realStats && realStats.totalTokens > 0) ? realStats.totalTokens : getTotalQuranOccurrences();
    ...
    var avgOccPerFoundationWord = totalOcc > 0 && FOUNDATION_WORDS.length > 0
      ? (typeof getRealAverageFoundationWordTokenCount === 'function' && getRealAverageFoundationWordTokenCount() > 0
          ? getRealAverageFoundationWordTokenCount()
 ...
 * Returns the REAL measured percentage (~21), or 0 if unavailable.
 */
function getFoundationTotalCoveragePercent() {
  // REAL coverage first: token-level union of foundation words / corpus tokens.
  var realPct = typeof getRealFoundationCoveragePercent === 'function' ? getRealFoundationCoveragePercent() : null;
  if (realPct !== null && realPct !== undefined) {
    return Math.round(realPct * 10) / 10;
  }
  // Legacy fallback (no corpus data / test environments)
...
var FOUNDATION_MILESTONE_MESSAGES = [
  ...
  { pct: 100, messages: [
    'Foundation Course Complete! You now understand approximately {comprehension}% of all word occurrences \u2014 recognizing approximately {coverage}% of all Quran word tokens.',
...
function getFoundationLessonContextMsg(lessonIndex) {
  ...
  // Resolve the real {coverage} placeholder in lesson copy (e.g. lesson 10's
  // "recognize approximately X% of all Quranic word occurrences") to the real
  // token-level Foundation coverage instead of the inflated legacy number.
  var context = lesson.lessonContext || '';
  if (context.indexOf('{coverage}') >= 0) {
    var realCoverage = typeof getFoundationTotalCoveragePercent === 'function' ? getFoundationTotalCoveragePercent() : 0;
    context = context.replace('{coverage}', String(Math.round(realCoverage * 10) / 10));
  }
  var displayStrings = typeof getFoundationLessonDisplayStrings === 'function'
    ? getFoundationLessonDisplayStrings(lessonIndex)
    : null;
  return {
    ...
    cumulativeMsg: lesson.cumulativeCoverageNum
      ? 'Cumulative: ' + (displayStrings ? displayStrings.cumulativeCoverage : lesson.cumulativeCoverage) + ' of Quranic occurrences'
      : '',
    totalOccurrences: totalOcc,
    rootCount: rootCount,
  };
...
/**
 * Real token-level coverage display strings for a foundation lesson
 * ("X.X%" of Quran word tokens), computed from actual corpus token
 * matching. Falls back to the stored (legacy) strings when the corpus
 * data is unavailable.
 */
function getFoundationLessonDisplayStrings(lessonIndex) {
  if (!FOUNDATION_LESSONS || lessonIndex === undefined || lessonIndex < 0 || lessonIndex >= FOUNDATION_LESSONS.length) {
    return null;
  }
  var lesson = FOUNDATION_LESSONS[lessonIndex];
  var lessonWords = typeof getFoundationLessonWords === 'function' ? getFoundationLessonWords(lessonIndex) : [];
  var cumWords = [];
  for (var li = 0; li <= lessonIndex; li++) {
    var lw = typeof getFoundationLessonWords === 'function' ? getFoundationLessonWords(li) : [];
    cumWords = cumWords.concat(lw);
  }
  var realLesson = typeof getRealWordsCoveragePercent === 'function' ? getRealWordsCoveragePercent(lessonWords) : null;
  var realCum = typeof getRealWordsCoveragePercent === 'function' ? getRealWordsCoveragePercent(cumWords) : null;
  return {
    lessonCoverage: realLesson !== null && realLesson !== undefined ? realLesson.toFixed(1) + '%' : (lesson.lessonCoverage || '0%'),
    cumulativeCoverage: realCum !== null && realCum !== undefined ? realCum.toFixed(1) + '%' : (lesson.cumulativeCoverage || '0%'),
  };
}
...
function getFoundationMilestoneMessage() {
  ...
  msg = msg.replace('{coverage}', String(typeof getFoundationTotalCoveragePercent === 'function' ? getFoundationTotalCoveragePercent() : 0));
...
function getAdaptiveRecommendation() {
  ...
  if (profile.studiedWords < 10 && typeof getFoundationLessonCount === 'function' && getFoundationLessonCount() > 0) {
    ...
    return {
      ...
      path: 'foundation',
      label: 'Foundation Course',
      reason: 'Start your Quran vocabulary journey! The Foundation Course teaches the 100 most frequent words, recognizing ~21% of all Quran word tokens. ' +
    ...
    if (fCompleted < fTotal) {
      ...
      return {
        ...
        reason: 'Foundation Course ' + progressPct + '% complete. Next lesson adds ~' + coverageBoostPct + '% Quran coverage. ' +
          'Complete all ' + fTotal + ' lessons for ~21% Quran word-token coverage.',
